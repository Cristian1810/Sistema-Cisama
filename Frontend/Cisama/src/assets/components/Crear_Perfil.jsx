...
export default function Crear_Perfil() {
  ...
  const fetchUsuarios = async () => {
    try {
      ...
      const response = await fetch('https://sistema-cisama-552k.onrender.com/usuarios', {
        headers: {
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editIdx !== null) {
        ...
        const response = await fetch(`https://sistema-cisama-552k.onrender.com/usuarios/${usuarioEdit.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handleDisable = async idx => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://sistema-cisama-552k.onrender.com/usuarios/${usuario.id}/disable`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleEnable = async idx => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://sistema-cisama-552k.onrender.com/usuarios/${usuario.id}/enable`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDelete = async idx => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`https://sistema-cisama-552k.onrender.com/usuarios/${usuario.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
