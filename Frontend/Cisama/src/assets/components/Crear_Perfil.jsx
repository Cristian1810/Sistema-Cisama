...
export default function Crear_Perfil() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (editIdx !== null) {
        ...
        const response = await fetch(`http://localhost:3000/usuarios/${usuarioEdit.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handleDisable = async idx => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`http://localhost:3000/usuarios/${usuario.id}/disable`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleEnable = async idx => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`http://localhost:3000/usuarios/${usuario.id}/enable`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
